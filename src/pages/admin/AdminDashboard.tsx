...
export const AdminDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filter Bar */}
      <div className="glass-card p-4 no-print border-violet-500/10">
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          ...
          <div className="flex flex-1 flex-wrap items-center gap-4">
            <div className="relative group">
              <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 w-4 h-4 group-focus-within:text-violet-400 transition-colors" />
              <input 
                type="date" 
                value={startDate}
                onChange={(e) => setStartDate(e.target.value)}
                className="bg-slate-900/50 border border-white/10 rounded-lg py-2 pl-9 pr-4 text-sm text-white focus:outline-none focus:border-violet-500/50 w-full md:w-auto"
              />
            </div>
            <span className="text-slate-600 hidden md:block">to</span>
            <div className="relative group">
              <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 w-4 h-4 group-focus-within:text-violet-400 transition-colors" />
              <input 
                type="date" 
          ...
          <div className="text-xs text-slate-500 italic block md:inline">
